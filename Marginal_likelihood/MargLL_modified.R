### Simulation study to investigate the performance of the newly proposed marginal likelihood 
### estimation algortihm using a state-independent proposal.
...
### Estimation of the log marginal likelihood (LML) ############################
################################################################################
## print true LML
print(c('true log marginal likelihood = ',trueLML));

### configurate the Estimation algorithm #############
# set choice of state-independent proposal function for LML estimation, 
# either 'kernel' or 'mvn'
  proposal <- 'kernel'  
  if (proposal=='kernel'){
    thinning <- 100;
    band <- (length(samplematrix[1,])/thinning)^(-2/(d+4))*cov(t(samplematrix));
    qfunc <- function(xval){
      return(mean(dmnorm(x=t(matrix(xval,nrow=d,ncol=length(seq(1,length(samplematrix[1,]),by=thinning)))), mean = t(samplematrix[,seq(1,length(samplematrix[1,]),by=thinning)]), varcov=band)))}
    qsample <- function(samplesize){
      return(rmnorm(n=samplesize, mean =samplematrix[,seq(1,length(samplematrix[1,]),by=thinning)[sample.int(n=length(seq(1,length(samplematrix[1,]),by=thinning)), 
                    size = samplesize, replace = TRUE)]], varcov=band))}
  }
  if (proposal=='mvn'){    
    qfunc <- function(xval){
      return(dmnorm(x=xval, mean = rowMeans(samplematrix), varcov=cov(t(samplematrix))))}
    qsample <- function(samplesize){
      return(rmnorm(n=samplesize, mean =rowMeans(samplematrix), varcov=cov(t(samplematrix))))}
  }
...
## first evaluate the proposal function q for all parameter vectors from the 
## (possibly thinned) posterior sample
  postsamplethin <- 1;
  psampleindex <- sample.int(n=length(samplematrix[1,]), size = length(samplematrix[1,])/postsamplethin, replace = FALSE);
  q_postsample <- rep(0,length(psampleindex));
  for (i in 1:length(q_postsample)){q_postsample[i] <- log(qfunc(samplematrix[,psampleindex[i]]))}
...
## loop over the number of overall evaluations M
  ...
  # generate a sample of size N from the proposal function including their 
  # corresponding proposal density values und unnormalized posterior values
    canpara <-  qsample(samplesize=N);
    q_cansample <- rep(0,length(canpara[,1]));
    if (model=='exponential'){   
      canpost     <- rep(0,length(canpara[,1]));
      for (i in 1:length(canpara[,1])){
        canpost[i] <- prior(canpara[i,])+LL(canpara[i,]);
        q_cansample[i] <- log(qfunc(canpara[i,]));}
    }else{
      canpost <-  prior(canpara)+LL(canpara);  
      for (i in 1:length(canpara[,1])){q_cansample[i] <- log(qfunc(canpara[i,]));}
    }

  # loop over the number of single estimator evaluations K             
    for (k in 1:K){
    # sample a parameter vector and compute its proposal log density value
      repeat{
        fixpara <-  qsample(samplesize=1);
        fixLL <- prior(fixpara)+LL(fixpara);
        if (fixLL>-Inf){break}
      }   
      logkerndens <- log(qfunc(fixpara));
    # compute the numerator 
      acceptsample <- fixLL-logkerndens+q_postsample-logposteriorsample[psampleindex]
      acceptsample <- (acceptsample-abs(acceptsample))/2;
      numerator <- mean(exp(acceptsample));
    # compute the demunerator  ########
      acceptprob <- -fixLL+logkerndens-q_cansample+canpost
      acceptprob <- (acceptprob-abs(acceptprob))/2;
      denumerator <- mean(exp(acceptprob));
    # compute the resulting log marginal likelihood estimate
      margLL[k] <- fixLL - logkerndens - log(numerator/denumerator);
    }
...
# save the overall LML estimator sample
  save(estMLL,file=paste('simresults_d',model,d,proposal,'K',K,'J',J,'.Rdata'));
